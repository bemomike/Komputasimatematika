...
close all;
%initialization
swarm_size = 64;                %jml partl
maxIter = 100;                  %jml iter
inertia = 1.0;
...
C = cat(2,X',Y');
D = reshape(C,[],2);
swarm(1:swarm_size,1,1:2) = D;  %posisi awal partl
swarm(:,2,:) = 0;               %kecptan awal partl
...
for iter = 1:maxIter
    ...
    swarm(:, 2, 1) = inertia*(rand(swarm_size,1).*swarm(:, 2, 2)) + correction_factor*(rand(swarm_size,1).*(swarm(:, 3, 2) ...
        - swarm(:, 1, 2))) + correction_factor*(rand(swarm_size,1).*(swarm(gbest, 3, 2) - swarm(:, 1, 2)));
    
    %plot partl
    clf;plot(swarm(:, 1, 1), swarm(:, 1, 2), 'bx');grid                                       %gerak swarm
    axis([-4 4 -4 4]);
    pause(.5);
